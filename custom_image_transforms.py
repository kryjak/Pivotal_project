...
class CustomTransforms:
    ...
    def __call__(self, image: t.Tensor) -> t.Tensor:
        ...
        if self.params.get('max_jitter_ratio') is not None:
            assert self.params['max_jitter_ratio'] > 0, 'max_jitter_ratio must be positive'
            mean_side = (h + w) / 2
            ...
            jitter_offset = t.stack([jitter_x.expand(h, w), jitter_y.expand(h, w)], dim=-1).unsqueeze(0)
        ...
        if self.params.get('contrast_range') is not None:
            assert len(self.params['contrast_range']) == 2, 'contrast_range must be a tuple of two numbers'
            assert self.params['contrast_range'][0] < self.params['contrast_range'][1], 'contrast_range[0] must be less than contrast_range[1]'
            contrast_factor = t.randn(1, device=device, dtype=dtype) * (self.params['contrast_range'][1] - self.params['contrast_range'][0]) + self.params['contrast_range'][0]
        ...
        if self.params.get('grayscale_prob') is not None:
            assert 0 <= self.params['grayscale_prob'] <= 1, 'grayscale_prob must be between 0 and 1'
