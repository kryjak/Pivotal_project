...
def _denormalise(image: t.Tensor, mean: Union[float, List[float]], std: Union[float, List[float]]) -> t.Tensor:
    return ((image * std) + mean)


# function to display the image processed with LlavaNextImageProcessor
def display_processed_tiles(pixel_values: t.Tensor, mean: Union[float, List[float]], std: Union[float, List[float]]) -> Image.Image:
    """
    ...
    for i in range(num_tiles):
        row = i // tiles_per_row
        col = i % tiles_per_row
        x = col * max_width + (col + 0) * 2  # Add 2 pixels for the frame around each tile
        y = row * max_height + (row + 0) * 2  # Add 2 pixels for the frame around each tile
        ...
        framed_image = Image.new('RGB', (image_pil.width + frame_width * 2, image_pil.height + frame_width * 2), frame_color)
        framed_image.paste(image_pil, (frame_width, frame_width))

        # Draw the frame around the image
        draw = ImageDraw.Draw(framed_image)
        draw.rectangle([(0, 0), (framed_image.width - 1, framed_image.height - 1)], outline=frame_color, width=frame_width)
