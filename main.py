...
# logits, answer_auto = multi_token_attack.execute_attack(test_prompt, init_image, delta,
#                                                    generation_method='automatic', max_new_tokens=5, no_eos_token=True,
#                                                    do_sample=True, top_p=0.95, top_k=20)
# print(f'{answer_auto=}')
# logits, answer_manual = multi_token_attack.execute_attack(test_prompt, init_image, delta,
...
augmentation_attack = JailbreakAttack(base_instance, jailbreak_config + wandb_config, wandb_name='augmentations')
delta, loss_train = augmentation_attack.train(prompts=train_goal_single, images=[img], targets=train_target_tokenized_single, training_method='teacher_forcing', use_cache=False, batch_size=1, augmentations=augmentations)
