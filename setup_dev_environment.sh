...
# 1. Ask if user wants to create a new folder
create_folder() {
    while true; do
...
# 2. Ask user for git clone method and clone the repository
clone_repo() {
    local repo_url
    while true; do
...
# 3. Check Linux distribution and ask user about editor installation
install_editor() {
...
echo "python -m ipykernel install --user --name=custom_venv --display-name='Custom Virtual Env'"
echo "-------------------"
