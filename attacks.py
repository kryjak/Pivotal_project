...
class ControlSingleTokenAttack():
    ...
    def train_attack(self,
                     ...
                     verbose: Optional[bool] = True) -> Tuple[t.Tensor, t.Tensor, List[float]]:
        ...
        target_tokenized = self.base.tokenizer.encode(target, add_special_tokens=False, return_tensors='pt').to(self.device)
    ...
    def execute_attack(self,
        ...
        if adversarial_image is None:
            if isinstance(delta, t.Tensor) and isinstance(image, t.Tensor):
                ...
                raise ValueError("Both image and delta must be tensors when adversarial_image is not provided")
    
        adversarial_image = adversarial_image * 255.0

        with t.no_grad():
            if generation_method == 'automatic':
                output, answer = self.base.generate_autoregressive(prompt, adversarial_image, max_new_tokens=max_new_tokens, no_eos_token=no_eos_token, **kwargs)
            elif generation_method == 'manual':
                output, answer = self.base.generate_autoregressive_manual(prompt, adversarial_image, max_new_tokens=max_new_tokens, no_eos_token=no_eos_token, **kwargs)
            else:
...
class ControlMultipleTokensAttack:
    ...
    def _init_wandb(self) -> None:
        run = wandb.init(project=self.cfg.wandb_project, id=self.wandb_run_id, name=self.wandb_name, config=self.cfg, resume='allow')
    ...
    def _forward_pass(self, prompt: Union[str, List[str]], perturbed_image: t.Tensor, target: List[str], training_method: str, use_cache: Optional[bool] = False, **kwargs) -> Tuple[t.Tensor, Dict, Dict]:
        loss_fn = t.nn.CrossEntropyLoss()
        ...
        for ii, target_tok in enumerate(target):
            ...
            output = self.base.generate_token_grad(prompt, perturbed_image, use_cache=use_cache, past_key_values=past_key_values, past_output=past_output, **kwargs)
            past_key_values = output.past_key_values

            logits = output.logits
            next_token_logits = logits[:, -1]
            next_token_pred = self.base.tokenizer.decode(next_token_logits.argmax(-1), skip_special_tokens=True)

            target_tokenized = self.base.tokenizer.encode(target_tok, add_special_tokens=False, return_tensors='pt').to(self.device)
            loss += loss_fn(next_token_logits, target_tokenized[:, -1])
    ...
    def train_attack(self,
        ...
        assert self.cfg.n_epochs > self.cfg.n_logs, 'For MultiTokenAttack, n_epochs must be greater than n_logs.'
    ...
    def execute_attack(self,
        ...
        if image == None and isinstance(delta, t.Tensor):
            adversarial_image = delta
        elif isinstance(delta, t.Tensor) and isinstance(image, t.Tensor):
            adversarial_image = (image + eps * delta.clamp(-1, 1))
        else:
            raise NotImplementedError(f"""You can either supply the adversarial image through 'delta', or the original image and its perturbation 'delta'.
        ...
        with t.no_grad():
            if generation_method == 'automatic_with_pil':
                output, answer = self.base.generate_autoregressive_with_pil(prompt, adversarial_image, max_new_tokens=max_new_tokens, no_eos_token=no_eos_token, use_cache=use_cache, **kwargs)
            elif generation_method == 'automatic':
                output, answer = self.base.generate_autoregressive(prompt, adversarial_image, max_new_tokens=max_new_tokens, no_eos_token=no_eos_token, use_cache=use_cache, **kwargs)
            elif generation_method == 'manual':
                output, answer = self.base.generate_autoregressive_manual(prompt, adversarial_image, max_new_tokens=max_new_tokens, no_eos_token=no_eos_token, use_cache=use_cache)
            else:
                raise NotImplementedError(f'Generation method {generation_method} not implemented yet.')
...
class JailbreakAttack(ControlMultipleTokensAttack):
    ...
    def train(self,
              ...
              verbose: bool = True,
              batch_size: int = 8,
              augmentations: Optional[dict] = None) -> Tuple[t.Tensor, List[float]]:
        ...
        for step in range(self.cfg.n_epochs):
            for batch_idx, batch in enumerate(dataloader):
                ...
                for ii, prompt in enumerate(batch_prompts):
                    ...
                    loss, next_token_preds, target_token_probs = self._forward_pass(prompt,
                                                                                    perturbed_image, target, training_method, use_cache=use_cache)
                    loss_batch += loss
    ...
    def execute_with_clamp(self,
        ...
        return super().execute_attack(prompt, image, delta, eps, generation_method, max_new_tokens, no_eos_token, use_cache, **kwargs)
    ...
    def test(self,
            ...
            delta: Optional[t.Tensor] = None,
            eps: Optional[float] = None,
            generation_method: Union[List[str], Literal['automatic_with_pil', 'automatic', 'manual', 'all']] = 'all',
            **kwargs
            ) -> Dict:

        if images is not None:
            assert len(images) == len(prompts), "If supplying images to be perturbed, number of prompts and images must be the same."
            n_images = len(images)
            print(f'Number of images: {n_images}')
            init_images = [transforms.ToTensor()(img).to(t.bfloat16).to(self.device) for img in images]
        else:
        ...
        for gen_method in generation_method:
            ...
            for prompt, init_image in zip(prompts, init_images):
                ...
                _, answer = self.execute_with_clamp(prompt, init_image, delta, eps, gen_method, **kwargs) # <-- clamping
                if type(answer) == list:
                    answer = ''.join(answer)
    ...
    def eval_dataset(self, **kwargs):
        ...
        for method in df.columns.difference(['goal', 'target']):
            ...
            for ind in df.index:
                ...
                prompt = self.cfg.jailbreak_eval_user_prompt.format(target=target, text=jailbreak_output)
                _, answer = self.base.generate_autoregressive(prompt=prompt, image=None, system_prompt=self.cfg.jailbreak_eval_system_prompt, **kwargs)
